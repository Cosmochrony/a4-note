...
\begin{document}
  ...
  = -2|\mathbf{E}_{\mathcal P}|^2$ on the electric branch, the radicand reduces to
    ...
    \begin{proposition}[Boundary-versus-interior selection]
      \label{prop:amplitude-dichotomy}
      On the electric branch the locked amplitude is governed by the sign of $\Pchi \cdot \mathcal{R}_{\chi}$ through an
      ...
      \]
      which defines an interior lock only if it lies before the first saturation contact, $\Dchi(s_*) > 0$.
      If $\Pchi \cdot \mathcal{R}_{\chi} \leq 0$ there is no quartic stabilisation, the radicand decreases
      monotonically at this order, and the amplitude is fixed by the first contact with the saturation bound,
      $\Dchi(s_*) = 0$.
    \end{proposition}
