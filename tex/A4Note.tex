...
\begin{document}
  ...
  \section{Position and purpose}
    ...
    backreaction $\Pchi \cdot \mathcal{R}_{\chi}$ and to a boundary-versus-interior selection of the Born--Infeld
    saturation problem.
  ...
  = -2|\mathbf{E}_{\mathcal P}|^2$ on the electric branch, the radicand reduces to
    ...
    What remains open is therefore narrow and explicit: the cubic response $\mathcal{R}_{\chi}$, hence the sign of
    $\Pchi \cdot \mathcal{R}_{\chi}$; the boundary-versus-interior selection it controls; and the normalisation
    $\mathcal{N}_A$ that converts $s_*$ into $|u|$.
  ...
  \section{Status of results}
    ...
    \paragraph{Conditional (this note).}
      Under the fixed-generator orbit Hypothesis~\ref{hyp:fixed-generator-orbit}, the cubic tilt collapses on the
      rank-one $\JPi$-odd outer-weight mode to $\mathcal{R}_{\chi} = -\tfrac{2}{3}|b|^2 \Pchi$, parallel to the
      polarisation, hence pure reparametrisation gauge (Remark~\ref{rem:conditional-sign}): the two-form trajectory is
      a straight ray and the locked amplitude is the first saturation contact, the boundary branch of
      Proposition~\ref{prop:amplitude-dichotomy}, not an interior lock.
      The hypothesis is not derived from the present corpus, and a genuine interior branch would require a
      $\Pchi$-transverse cubic response, part of the open path-curvature datum.

    \paragraph{Open.}
      The amplitude $|u|$ is narrowed by Section~\ref{sec:amplitude} but not closed.
      On the electric branch it reduces to the cubic response $\mathcal{R}_{\chi}$ of the chiral two-form, hence to the
      sign of the backreaction $\Pchi \cdot \mathcal{R}_{\chi}$ and the boundary-versus-interior selection of the
      Born--Infeld saturation problem (Proposition~\ref{prop:amplitude-dichotomy}), together with the discrete
      chiral-frontier normalisation $\mathcal{N}_A$ that maps the locked modulus $s_*$ to $|u|$.
