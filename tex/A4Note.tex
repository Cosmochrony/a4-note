...
\begin{document}
  ...
  \begin{abstract}
    ...
    The identification of the Schur projector tilt with the Born--Infeld saturation modulus on the A4 locus is discharged to
    a
    tangential coincidence, conditional only on Schur transversality; the Lorentzian genus computation is deliberately
    deferred.
  \end{abstract}
  ...
  \section{Position and purpose}
    ...
    Section~\ref{sec:coincidence} discharges the coincidence lock to tangential coincidence on the A4 locus, conditional
    only
    on Schur transversality, and isolates the Lorentzian genus as the next computation.
  ...
  \section{Discharge of the coincidence lock}
    ...
    \begin{proposition}[Discharge dichotomy]
      ...
      \begin{itemize}
        ...
        \item if the locking is Schur-null, then $\Pchi = 0$ and $\muchi = 0$ at quadratic order, which is the marginal branch
        of the trichotomy of Theorem~\ref{thm:reduction}: the locking character is then decided by the first non-vanishing
        even
        coefficient $\lambda_{2k}$, not directly by $u = 0$.
      \end{itemize}
    \end{proposition}

    The coincidence lock is thereby discharged to structural status, conditional only on Schur transversality
    (Definition~\ref{def:transversality}); by Corollary~\ref{cor:reparam} the eventual genus does not depend on the positive
    Jacobian $\phi'(0)$.
  ...
  \section{Status of results}
    ...
    \paragraph{Conditional (sole residual).}
      Schur transversality on the $\JPi$-odd eliminated sector (Definition~\ref{def:transversality}): the non-vanishing of the
      Schur transport on the direction opened by $\partial_s P|_{0}$.
      If it holds, the coincidence lock is discharged and the genus computation is the next step; if it fails, the quadratic
      term vanishes and the locking character falls to the marginal branch of Theorem~\ref{thm:reduction}
    ...
    \paragraph{Open.}
      ...
      coincidence lock is discharged modulo Schur transversality.
